...
fun Activity.getFileNameFromUri(uri: Uri, ext: String): Uri {
    val path = uri.path ?: ""
    val segments = path.split("/")
    val fileName = if (segments.isNotEmpty()) {
        segments.last()
    } else {
        ""
    }
    val uniqueFileName = "$fileName.$ext"
    return Uri.fromFile(File(cacheDir, uniqueFileName))
}
